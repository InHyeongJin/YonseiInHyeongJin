...
저는 약 3년간 해외 주식 투자를 경험하면서 주가의 변동성과 예측의 어려움을 직접 체감해왔습니다. 이번 학기 수학과프로그래밍 수업에서 배운 파이썬을 활용한 데이터 시각화 기법(mathplotlib)이 매우 흥미로웠고 이는 제가 직접 주가 그래프를 만들고 싶다는 생각으로 이어졌습니다. 또한 반복되는 수열 구조를 분석하는 점화식 개념 역시 인상 깊었습니다. 이러한 요소들을 실제 투자 시뮬레이션에 접목시켜 보고자, 점화식을 이용한 단순한 주가 예측 모델을 만들고, 이를 기반으로 자동 매매 전략을 구현해 시각화하는 통합 프로젝트를 진행하게 되었습니다.
